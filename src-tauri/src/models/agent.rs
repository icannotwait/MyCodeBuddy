...
pub const BUILTIN_AGENT_WIRE_NAMES: &[&str] = &[
    "claude_code",
    "codex",
    "open_code",
    "gemini",
...
pub const BUILTIN_AGENT_REGISTRY_IDS: &[&str] = &[
    "claude-acp",
    "codex-acp",
    "gemini",
    "opencode",
...
/// Alternate ids used by the public ACP registry for integrations codeg ships
/// under a different canonical registry id.
pub const BUILTIN_AGENT_REGISTRY_ALIASES: &[&str] = &["kimi"];

pub fn is_reserved_builtin_agent_id(id: &str) -> bool {
...
///
/// The eleven named variants are compile-time built-ins with hand-written
...
/// The eleven compile-time agents, in declaration order. Does NOT include
...
impl AgentType {
    ...
    pub fn from_wire(s: &str) -> Option<Self> {
        Self::from_untrusted_wire_with(s, |id| Some(crate::intern::intern(id)))
    }

    /// Parse request/event input without allowing arbitrary custom ids to grow
    /// the process-lifetime interner indefinitely. Already-registered ids are
    /// served by the trusted pool; unregistered ids share a fixed-size pool.
    pub fn from_untrusted_wire(s: &str) -> Option<Self> {
        Self::from_untrusted_wire_with(s, crate::intern::intern_custom_wire_id)
    }

    fn from_untrusted_wire_with(
        s: &str,
        intern_unregistered: impl FnOnce(&str) -> Option<&'static str>,
    ) -> Option<Self> {
        match s {
            ...
            other => other.strip_prefix(CUSTOM_AGENT_WIRE_PREFIX).and_then(|id| {
                if !is_valid_custom_agent_id(id) {
                    return None;
                }
                crate::intern::get(id)
                    .or_else(|| intern_unregistered(id))
...
/// conservative lowercase slug so it can never escape a directory, collide
/// case-insensitively in the cache, use the wire separator, or shadow a
/// built-in name.
pub fn is_valid_custom_agent_id(id: &str) -> bool {
    !id.is_empty()
        ...
        && !id.starts_with('.')
        // Wire names, ACP registry ids, and published aliases all participate
        // in built-in lookup/cache namespaces and therefore cannot be custom.
        && !is_reserved_builtin_agent_id(id)
}
...
impl<'de> Deserialize<'de> for AgentType {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        AgentType::from_untrusted_wire(&raw)
...
mod tests {
    ...
    fn unknown_wire_forms_are_rejected() {
        ...
        assert_eq!(AgentType::from_wire(&("open_".to_owned() + "claw")), None);
    ...
    fn custom_id_rejects_path_traversal_and_builtin_shadowing() {
        ...
        for reserved in BUILTIN_AGENT_WIRE_NAMES
            .iter()
            .chain(BUILTIN_AGENT_REGISTRY_IDS)
            .chain(BUILTIN_AGENT_REGISTRY_ALIASES)
        {
            assert!(
                !is_valid_custom_agent_id(reserved),
                "built-in identity {reserved:?} must be reserved"
            );
            assert_eq!(AgentType::custom(reserved), None);
        }
    ...
    #[test]
    fn trusted_wire_parsing_keeps_deleted_custom_ids_readable() {
        let agent = AgentType::from_wire("custom:deleted-history-agent").unwrap();
        assert_eq!(agent.custom_id(), Some("deleted-history-agent"));
    }

    #[test]
    fn untrusted_custom_wire_parsing_is_bounded() {
        let mut pool = crate::intern::BoundedInterner::new(2);
        let mut parse = |wire| AgentType::from_untrusted_wire_with(wire, |id| pool.intern(id));
        assert_eq!(
            parse("custom:bounded-wire-one"),
            Some(AgentType::Custom("bounded-wire-one"))
        );
        assert_eq!(
            parse("custom:bounded-wire-two"),
            Some(AgentType::Custom("bounded-wire-two"))
        );
        assert_eq!(
            parse("custom:bounded-wire-one"),
            Some(AgentType::Custom("bounded-wire-one"))
        );
        assert_eq!(parse("custom:bounded-wire-three"), None);
