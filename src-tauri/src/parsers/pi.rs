...
fn session_name(entries: &[SessionRecord]) -> Option<String> {
    entries
        ...
        .and_then(|name| visible_title(Some(name)))
}
...
fn parse_message_record(sp: &mut SessionParse, value: &Value, ts: DateTime<Utc>, idx: usize) {
    ...
    match role {
        "user" => {
            ...
            let text = visible_user_text(&text).unwrap_or_default();
    ...
    fn hides_codeg_terminal_context_from_history_and_title() {
        ...
        let detail = parse_one(
            &[
                ...
                json!({"type":"message","id":"m0","parentId":"i1","timestamp":"2026-06-27T10:00:01.000Z",
                       "message":{"role":"user","content":[{"type":"text","text":ctx}]}}),
                json!({"type":"message","id":"m1","parentId":"m0","timestamp":"2026-06-27T10:00:02.000Z",
                       "message":{"role":"user","content":[{"type":"text","text":real_plus}]}}),
                json!({"type":"message","id":"m2","parentId":"m1","timestamp":"2026-06-27T10:00:03.000Z",
