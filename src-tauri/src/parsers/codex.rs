...
impl CodexParser {
    ...
    fn rollout_lines(&self, path: &std::path::Path) -> Result<Vec<String>, ParseError> {
        ...
        self.rollout_lines_inner(path, &bytes, MAX_FORK_HOPS)
    }
    ...
    fn rollout_lines_inner(
        ...
        let own: Vec<String> = codex_complete_records(bytes)
            .filter_map(|(_, record)| {
                std::str::from_utf8(codex_record_payload(record))
                    .ok()
                    .map(str::to_owned)
            })
            .collect();
        ...
        let parent = self.rollout_lines_inner(&parent_path, &parent_bytes, hops_left - 1)?;
...
impl CodexParser {
    fn parse_conversation_detail(
        ...
        let own_bytes = fs::read(path)?;
        let transcript_watermark = codex_complete_records(&own_bytes)
            .map(|(start, record)| start + record.len() as u64)
            .last()
            .unwrap_or(0);
        let lines = self.rollout_lines_inner(path, &own_bytes, MAX_FORK_HOPS)?;
        ...
        detail.transcript_watermark = Some(transcript_watermark);
        Ok(detail)
    }
                    ...
                    if let Some(payload) = value.get("payload") {
                        ...
                        match payload_type {
                            ...
                            "user_message" => {
                                ...
                                if let Some(visible) =
                                    visible.as_ref().filter(|text| !text.is_empty())
                                {
