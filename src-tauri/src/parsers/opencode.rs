...
impl OpenCodeParser {
    ...
    async fn get_conversation_from_sqlite(
        ...
        if summary.title.is_none() {
            if let Some(text) = turns.iter().find_map(|turn| {
                ...
                summary.title = Some(title_from_user_text(text));
            }
        }
...
fn resolve_title(title: Option<String>, first_user_text: Option<String>) -> Option<String> {
    let first_user_text = first_user_text.and_then(|t| visible_user_text(&t));
    let Some(title) = visible_title(normalize_optional_string(title)) else {
    ...
    fn hides_codeg_terminal_context_from_history_and_title() {
        ...
        rt.block_on(async {
            let conn = Database::connect(format!("sqlite:{}?mode=rwc", db_path.display()))
                ...
                .expect("open");
            for ddl in [
                "CREATE TABLE session (id TEXT PRIMARY KEY, directory TEXT, parent_id TEXT, title TEXT, \
            ...
            conn.execute(Statement::from_sql_and_values(
                ...
                "INSERT INTO session (id, directory, parent_id, title, time_created, time_updated) \
                 VALUES (?, ?, ?, ?, ?, ?)",
                [
                    session_id.into(),
                    "/tmp/demo".into(),
