...
pub(crate) fn resolve_deepseek_sessions_root_for_runtime_env(
    ...
    Some(resolve_dsh_home_for_runtime_env(runtime_env)?.join("sessions"))
}

/// Resolve the DeepSeek child process's DSH_HOME using the same environment
/// inheritance and home-expansion rules as its session root.
fn resolve_dsh_home_for_runtime_env(runtime_env: &BTreeMap<String, String>) -> Option<PathBuf> {
    ...
    match dsh_home {
        Some(value) => {
            ...
            if needs_home {
                ...
                Some(expand_home_prefix(&value, Some(&child_home)))
            } else {
                Some(expand_home_prefix(&value, None))
            }
        }
        None => Some(crate::acp::file_system_runtime::child_home_dir(runtime_env)?.join(".dsh")),
    }
}
...
impl DeepSeekParser {
    pub fn new() -> Self {
        Self {
            ...
            attachments_root: Some(resolve_deepseek_attachments_root()),
        }
    }

    pub(crate) fn from_runtime_env(runtime_env: &BTreeMap<String, String>) -> Option<Self> {
        let base_dir = resolve_deepseek_sessions_root_for_runtime_env(runtime_env)?;
        let attachments_root = resolve_dsh_home_for_runtime_env(runtime_env)
            .map(|root| root.join("attachments").join("v1"));
        Some(Self {
            base_dir,
            attachments_root,
        })
    ...
    pub fn with_base_dir(base_dir: PathBuf) -> Self {
        let attachments_root = Some(base_dir.join("attachments").join("v1"));
        Self {
            base_dir,
            attachments_root,
        }
    ...
    fn build_detail(&self, session_dir: &Path, conversation_id: &str) -> ConversationDetail {
        let parsed =
            parse_session_log(session_dir, self.attachments_root.as_deref()).unwrap_or_default();
