...
fn session_summary(
    ...
    let title = visible_title(non_empty(
        manifest
            .metadata
            .as_ref()
    ...
    .or_else(|| {
        non_empty(manifest.prompt.clone())
            ...
            .and_then(|prompt| visible_user_text(&prompt))
    })
...
impl AgentParser for ClineParser {
    fn list_conversations(&self) -> Result<Vec<ConversationSummary>, ParseError> {
        ...
        summaries.extend(self.list_legacy_tasks(true)?);
        Ok(summaries)
    }
    ...
    fn recover_conversation(
        ...
        let mut summaries = self.list_session_store();
        summaries.extend(self.list_legacy_tasks(false)?);
    ...
    fn list_legacy_tasks(
        ...
        for entry in entries {
            ...
            let message_count = if read_message_counts {
                match self.read_api_history_raw(&tasks_dir) {
                    Ok(raw) => serde_json::from_str::<Vec<serde_json::Value>>(&raw)
                        .ok()
...
fn parse_session_user_parts(content: &serde_json::Value) -> UserMessageParts {
    ...
    let user_blocks = texts
        ...
        .filter_map(|text| {
            ...
            visible_user_text(&cleaned).map(|text| ContentBlock::Text { text })
        })
        .collect();
