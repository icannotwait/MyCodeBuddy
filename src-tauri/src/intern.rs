...
/// Maximum number of distinct, otherwise-valid custom ids accepted directly
/// from wire input before registration. Registered definitions use [`intern`]
/// and remain unaffected by this defensive limit.
const MAX_UNTRUSTED_CUSTOM_WIRE_IDS: usize = 1024;
...
/// Return a value previously promoted by the trusted metadata interner.
...
/// Fixed-capacity interner used for valid but unregistered custom ids arriving
/// through a wire boundary. Once full it reuses known values and rejects new
/// ones, so arbitrary request bodies cannot cause process-lifetime growth.
pub(crate) struct BoundedInterner {
    limit: usize,
...
impl BoundedInterner {
    ...
    pub(crate) fn intern(&mut self, s: &str) -> Option<&'static str> {
        if let Some(found) = self.values.get(s) {
            return Some(*found);
        }
...
/// Intern an unregistered custom id from a wire representation, subject to a
/// process-wide cardinality ceiling.
pub fn intern_custom_wire_id(s: &str) -> Option<&'static str> {
    static WIRE_IDS: OnceLock<Mutex<BoundedInterner>> = OnceLock::new();
    WIRE_IDS
        .get_or_init(|| Mutex::new(BoundedInterner::new(MAX_UNTRUSTED_CUSTOM_WIRE_IDS)))
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .intern(s)
}
