...
mod tests {
    ...
    #[tokio::test]
    async fn upsert_rejects_builtin_registry_ids_and_aliases() {
        let db = fresh_in_memory_db().await;
        for reserved in [
            "claude_code",
            "codex",
            ...
            let err = upsert(&db.conn, &npx_def(reserved))
                .await
                .expect_err("built-in identity must not be persisted as custom");
            assert!(
                err.to_string().contains("invalid custom agent id"),
