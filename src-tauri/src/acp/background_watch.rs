...
use crate::parsers::claude::{
    capture_tag, capture_title_record, find_session_file, group_into_turns,
    is_explicit_automation_marker, is_meta_message, slash_command_display,
...
impl PromptLedger {
    ...
    /// `connection.rs`).
    ///
    /// Also a test convenience over [`Self::record_prompt_blocks`]. (The
    /// `_session/steering` arm in `connection.rs` used to be the production
    /// caller; it now records the steered blocks directly, since a steered
    /// draft can carry attachments.)
    #[allow(dead_code)]
...
impl WatchState {
    ...
    pub(crate) fn tick(
        ...
        for settlement in settled.iter_mut() {
            settlement.wire_visible = self.current_turn_launched_ids.contains(&settlement.task_id);
        }
                    ...
                    if trimmed.starts_with("<task-notification>") {
                        ...
                        if let Some(id) = task_id {
                            ...
                            settled.push(BackgroundSettledInfo {
                                ...
                                tool_use_id,
                                result,
                                // Classified after the full watcher batch has
                                // been read, while the held-turn id set is known.
                                wire_visible: false,
                            });
                        }
                    }
