...
fn codex_distribution() -> AgentDistribution {
    ...
    // `thread/reverted` + `thread/queue/changed` join the ignored-notification
    // list, and `misalignmentPolicyViolation` maps onto the existing
    // `policy_denied` bucket.
    //
    // 1.7.0 rebuilds permissions and redefines the approval presets. Request
    // `_meta.permission` now carries the reason, option-level `changes[]` is
    // gone, and `read-only` now means workspace-write with user-reviewed
    // approvals while `agent` uses auto-review. Existing generic permission
    // parsing handles the standard ACP fields and request metadata.
    //
    // Native ACP subagent sessions remain disabled: the schema crate cannot
    // represent the capability or deserialize the new session updates. File
