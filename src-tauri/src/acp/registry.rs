...
pub struct AcpAgentMeta {
    ...
    /// 空列表 `[]` 仍会按 ACP schema 序列化；the gate only omits entries when
    /// an agent cannot accept per-session MCP servers.
...
pub fn get_agent_meta(agent_type: AgentType) -> AcpAgentMeta {
    ...
    match agent_type {
        ...
        AgentType::Pi => AcpAgentMeta {
            ...
            // `true` so every built-in stays on the MCP path; actual
            // wire forwarding is short-circuited in
    ...
    fn codex_is_npx_on_all_platforms() {
        match codex_distribution() {
            AgentDistribution::Npx {
                ...
                assert_eq!(package, "@agentclientprotocol/codex-acp@1.12.0");
