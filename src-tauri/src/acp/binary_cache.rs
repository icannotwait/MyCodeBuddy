...
pub(crate) fn cache_dir() -> Result<PathBuf, AcpError> {
    ...
    Ok(base.join("app.mycodebuddy").join("acp-binaries"))
}
...
pub fn uvx_prepared_version(agent_type: AgentType) -> Option<String> {
    let path = uvx_prepared_marker(&agent_cache_key(agent_type)).ok()?;
...
pub fn mark_uvx_agent_prepared(agent_type: AgentType, version: &str) -> Result<(), AcpError> {
    let path = uvx_prepared_marker(&agent_cache_key(agent_type))?;
...
pub(crate) fn agent_cache_key(agent_type: AgentType) -> String {
    match agent_type {
        AgentType::Custom(id) => format!("custom-{id}"),
        builtin => registry::registry_id_for(builtin).to_string(),
    }
}
...
fn dir_entry_for_agent_id(agent_id: &str) -> Option<registry::BinaryDirEntry> {
    let meta = if let Some(custom_id) = agent_id.strip_prefix("custom-") {
        crate::acp::custom_registry::get(custom_id)?.clone()
    } else {
        registry::get_agent_meta(registry::from_registry_id(agent_id)?)
    };
...
mod tests {
    ...
    fn cache_key_uses_registry_id() {
        ...
        assert_eq!(
            agent_cache_key(AgentType::custom("goose").unwrap()),
            "custom-goose"
        );
        assert_ne!(
            agent_cache_key(AgentType::Custom("codex-acp")),
            agent_cache_key(AgentType::Codex),
            "even an illegally constructed custom variant must not share a built-in cache key"
        );
    }
