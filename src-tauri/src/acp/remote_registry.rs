...
/// Whether an id is one of codeg's eleven hand-written agents. Deliberately
...
fn is_builtin_registry_id(id: &str) -> bool {
    registry::is_reserved_builtin_id(id)
}
...
mod tests {
    ...
    fn builtin_ids_are_recognized_without_the_custom_registry() {
        for id in crate::models::agent::BUILTIN_AGENT_REGISTRY_IDS {
            assert!(
                is_builtin_registry_id(id),
                "canonical id {id} must be reserved"
            );
        }
