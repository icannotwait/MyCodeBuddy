...
pub async fn acp_fetch_registry_catalog_core(
    ...
    acp_fetch_registry_catalog_with_core(db, |installed| async move {
...
/// Catalog core with the external fetch supplied by the caller. Production
/// passes the public ACP registry fetcher; deterministic integration tests pass
/// an in-memory double while still exercising installed-id collection.
pub async fn acp_fetch_registry_catalog_with_core<F, Fut>(
    db: &AppDatabase,
    fetch: F,
) -> Result<Vec<RegistryCatalogAgent>, AcpError>
where
    F: FnOnce(Vec<String>) -> Fut,
    Fut: std::future::Future<Output = Result<Vec<RegistryCatalogAgent>, AcpError>>,
{
...
pub async fn acp_add_registry_agent_core(
    ...
    if registry::is_reserved_builtin_id(&registry_id) {
        return Err(AcpError::protocol(format!(
    ...
    acp_add_registry_agent_from_catalog_core(registry_id, kind, &catalog, db, emitter).await
}

/// Add from an already-fetched catalog. This is the non-network part of the
/// registry-add core and is shared with deterministic transport parity tests.
pub async fn acp_add_registry_agent_from_catalog_core(
    registry_id: String,
    distribution_kind: Option<CustomDistributionKind>,
    catalog: &[RegistryCatalogAgent],
    db: &AppDatabase,
    emitter: &EventEmitter,
) -> Result<(), AcpError> {
    if registry::is_reserved_builtin_id(&registry_id) {
    ...
    let kind = remote_registry::effective_kind(entry, distribution_kind)
        .map_err(|e| AcpError::protocol(e.to_string()))?;
