...
pub async fn token_usage_sync_core(
    ...
    registry: &crate::auto_title::InternalAgentSessionRegistry,
    ...
    let mut parsed = stream::iter(stale.into_iter().map(|candidate| async move {
        let detail = get_folder_conversation_core(conn, registry, candidate.id).await;
        (candidate, detail)
    }))
...
pub async fn token_usage_sync(
    app: tauri::AppHandle,
    db: tauri::State<'_, crate::db::AppDatabase>,
    registry: tauri::State<'_, std::sync::Arc<crate::auto_title::InternalAgentSessionRegistry>>,
    mode: Option<TokenUsageSyncMode>,
) -> Result<TokenUsageSyncResult, AppCommandError> {
    let emitter = EventEmitter::Tauri(app);
    token_usage_sync_core(
        &db.conn,
        &emitter,
...
mod tests {
    ...
    ) -> std::sync::Arc<crate::auto_title::InternalAgentSessionRegistry> {
        crate::auto_title::InternalAgentSessionRegistry::empty(
            db.conn.clone(),
            &std::env::temp_dir(),
        )
