...
pub async fn prepare_source_core(
    ...
    if encrypted && passphrase.is_none_or(|p| p.is_empty()) {
        // Locked preview only — inspect does not decrypt. The passphrase
        // path must not call inspect on the ciphertext: that would decrypt
        // a second full-size temporary beside the prepared copy.
        return Ok(PreparedSource {
            source_id: None,
            preview: core::inspect_backup_core(src, passphrase).await?,
        });
    }
...
async fn preview_zip(zip_path: &Path, encrypted: bool) -> Result<BackupPreview, AppCommandError> {
    // The zip is already plaintext (prepare decrypted it, or the caller
    // handed us an unencrypted archive). Reuse inspect for compat +
    // validate_manifest so the fork helper stays a live caller after the
    // source_id cutover. Overlay `encrypted`: inspect sees the decrypted file.
    let mut preview = core::inspect_backup_core(zip_path, None).await?;
