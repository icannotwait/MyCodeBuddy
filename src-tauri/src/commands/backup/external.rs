...
/// Pack caller-resolved sources (DeepSeek runtime env, tests) with the same
/// SQLite snapshot / progress path as [`add_external_sources`].
pub(crate) fn add_external_sources_with(
    builder: &mut ArchiveBuilder,
    scratch: &Path,
...
pub(crate) fn scan_external_conflicts(
    zip_path: &Path,
...
mod tests {
    use super::*;
    use crate::parsers::external_transcript_sources;
