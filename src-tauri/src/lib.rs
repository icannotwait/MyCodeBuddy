...
mod tauri_app {
    ...
    macro_rules! generate_production_invoke_handler {
        ($( $(#[$attribute:meta])* $command:path ),* $(,)?) => {
    ...
    macro_rules! production_tauri_commands {
        ($consumer:ident) => {
            $consumer![
                ...
                logging_commands::record_frontend_turn_trace,
    ...
    #[cfg(test)]
    macro_rules! collect_production_tauri_command_paths {
        ($( $(#[$attribute:meta])* $command:path ),* $(,)?) => {
            &[
                $(
                    $(#[$attribute])*
    ...
    #[cfg(test)]
    fn production_tauri_command_paths() -> &'static [&'static str] {
        production_tauri_commands!(collect_production_tauri_command_paths)
    }
    ...
    fn notify_web_auto_start_failed(
        ...
        tauri::async_runtime::spawn(async move {
            let _ = notification::send_notification(
                app,
                "DrawCode Web service".to_string(),
                body,
                None,
                None,
                None,
    ...
    pub fn run() {
        ...
        // they still share other `app.mycodebuddy` data-dir artifacts with release.
                ...
                let broker_for_lifecycle = {
                    ...
                    let reconcile_result = tauri::async_runtime::block_on(async move {
                        ...
                        crate::commands::system_settings::apply_persisted_terminal_shell_config(
                            &db_for_init,
                            &cm_for_watchdog.terminal_shell_config(),
                        )
                        .await;
