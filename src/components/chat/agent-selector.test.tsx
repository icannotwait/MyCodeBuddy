...
describe("AgentSelector", () => {
  ...
  it("collapses the agents that don't fit into the more popover", () => {
    ...
    // 32px for the ⋯ button leaves room for 5 of the 11 unselected agents.
    stubLayout()
    mockUseAcpAgents.mockReturnValue({
      agents: MANY_AGENTS.map((type) => agent(type)),
      fresh: true,
    ...
    expect(screen.getByLabelText("More agents (6)")).toBeInTheDocument()
  })
  ...
  it("selects a collapsed agent from the more popover", async () => {
    ...
    fireEvent.click(screen.getByLabelText("More agents (6)"))
