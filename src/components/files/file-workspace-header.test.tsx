...
describe("FileWorkspaceHeader snapshot actions", () => {
  it("exposes no path-derived preview or browser action for an HTML-path snapshot", async () => {
    mocks.activeFileTab = snapshotHtmlTab(true)
    ...
    // A snapshot still has a path to copy, but must not offer a system-browser
    // open of the HTML-looking location — that path is not the image bytes.
    await openMoreMenu()
    expect(
      screen.queryByRole("menuitem", { name: "Open in system browser" })
    ).toBeNull()
  ...
  it("keeps HTML actions for ordinary tabs and restores them after conversion", async () => {
    mocks.activeFileTab = snapshotHtmlTab(false)
