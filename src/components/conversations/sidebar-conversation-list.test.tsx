...
describe("SidebarConversationList — Recent section", () => {
  ...
  it("collapses independently of the other sections", () => {
    ...
    expect(document.body.textContent).toContain("conv-11")
    expect(
      document.querySelectorAll('[data-sidebar-row-key^="conv-"]')
    ).toHaveLength(2)
    expect(
  ...
  describe("paging", () => {
    ...
    // Count the virtualized Recent rows, not nested card metadata or canonical
    // copies of the same conversations.
    const recentRowCount = () =>
      document.querySelectorAll('[data-sidebar-row-key^="recent-conv-"]').length
    ...
    it("offers the reset at the row's right edge while pages remain", () => {
      ...
      expect(recentRowCount()).toBe(PAGE * 2)
