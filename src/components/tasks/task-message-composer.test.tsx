...
vi.mock("@/components/chat/composer/use-reference-search", async () => {
  const { FakeReferenceSearchController: Fake } =
    await import("@/components/chat/composer/suggestion/fake-reference-search-controller")
  type Snapshot =
    import("@/components/chat/composer/reference-search-controller").ReferenceSearchSnapshot
  const emptyGroup = (
    kind: "agent" | "file" | "session" | "commit",
    label: string
  ): Snapshot["groups"]["file"] => ({
    kind,
    label,
    items: [],
    loading: false,
  ...
  const cannedFileSnapshot = (query = "app"): Snapshot => ({
    query,
    generation: 1,
    patternError: false,
    groups: {
  ...
  const controller = new Fake(cannedFileSnapshot())
  const originalSetQuery = controller.setQuery
  controller.setQuery = (query: string) => {
    controller.publish(cannedFileSnapshot(query))
    originalSetQuery(query)
  }
  return {
    useReferenceSearchController: () => controller.asController(),
  }
})
