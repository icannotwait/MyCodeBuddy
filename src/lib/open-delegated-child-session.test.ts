...
describe("openDelegatedChildSession", () => {
  ...
  it("no-ops when resolve fails", async () => {
    ...
    expect(peekDelegatedChildTabIntent(1)).toBeNull()
  })

  it("stages intent before openTab and keeps it when open succeeds", async () => {
    ...
    await expect(
      openDelegatedChildSession({
        ...
        childTurnAnchor: "turn-abc",
      })
    ).resolves.toBe(true)

    expect(openTab).toHaveBeenCalledTimes(1)
    expect(openTab).toHaveBeenCalledWith(7, 99, "codex", false, "Child title")
    expect(peekDelegatedChildTabIntent(99)).toEqual({
      focusTurnAnchor: "turn-abc",
      kickoffTask: "do the work",
      liveOwnsActiveTurn: true,
    })
  })

  it("clears intent when openTab fails", async () => {
    const openTab = vi.fn(async () => false)
    useTabStore.setState({ openTab } as never)
    useAppWorkspaceStore.setState({
      conversations: [childSummary],
    } as never)
    ...
    expect(peekDelegatedChildTabIntent(99)).toBeNull()
  })

  it("resolves agentType from workspace summary when omitted", () => {
    expect(
      resolveDelegatedChildOpenTarget({
        childConversationId: 99,
        agentType: null,
