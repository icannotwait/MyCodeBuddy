...
function focusTab(tabId: string) {
  ...
  if (state.activeTabId !== tabId) {
    useTabStore.setState({
      rawTabs: stampActiveTab(state.rawTabs, tabId),
      activeTabId: tabId,
      sidebarSelection: null,
    })
  } else if (state.sidebarSelection != null) {
    useTabStore.setState({ sidebarSelection: null })
  }
...
export const useTabStore = create<TabStoreState>()((set, get) => ({
  ...
  openTab: async (
    ...
    if (pin) {
      const next = insertTab(prevState.rawTabs, newTab, opts?.index)
  ...
  closeTab: (tabId, options) => {
    ...
    if (index >= 0) {
      ...
      if (next.length === 0) {
        ...
        set({
          rawTabs: stampActiveTab(next, newActiveId),
  ...
  closeOtherTabs: (tabId) => {
    ...
    set({
      rawTabs: stampActiveTab(keep, tabId),
      activeTabId: tabId,
      sidebarSelection: null,
    })
    recomputeTabs()
  ...
  openNewConversationTab: (folderId, workingDir, options) => {
    ...
    if (!existingTab) {
      ...
      const next = insertTab(prevState.rawTabs, newTab, options?.index)
      const keep = buildTabLimitKeepIds(next, [tabId])
      const { tabs: limited } = evictTabsToLimit(next, { keepTabIds: keep })
      set({
        rawTabs: stampActiveTab(limited, tabId),
...
function applyRemoteSnapshot(change: TabsChanged) {
  ...
  if (nextTabs.length === 0) {
    ...
    lastSavedPayload = diverged
      ? null
      : JSON.stringify(buildPersistItems(stampedReplacement, replacement.id))
    useTabStore.setState({
