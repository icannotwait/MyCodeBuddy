...
For an upstream Codeg `0.31.0` sync, reset the fork version in `package.json`,
`src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, and
`src-tauri/tauri.conf.json` to `0.31.0-mycodebuddy.1`. Run
`pnpm test:release` to verify that the versions and runtime URLs remain
consistent. The conflict cookbook for this merge is
`docs/UPSTREAM_0.31.0_CONFLICT_RESOLUTION.md`.
