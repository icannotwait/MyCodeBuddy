...
git switch -c sync/codeg-0.30.1
...
For an upstream Codeg `0.30.1` sync, reset the fork version in `package.json`,
`src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, and
`src-tauri/tauri.conf.json` to `0.30.1-mycodebuddy.1`. Run
