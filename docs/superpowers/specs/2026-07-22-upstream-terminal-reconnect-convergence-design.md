...
row from the workspace store and derive `isTerminalCancelled` from the backend
status. A missing summary is treated as not ready for automatic connection,
not as permission to reconnect. This avoids a refresh race in which the ACP
hook starts a historical session before the status patch or reconciliation
fetch arrives.

The status patch remains the source of truth. The per-connection
...
- **Automatic connection**: allowed for an active, non-terminal conversation
...
not reconnect a cancelled conversation. The surface will expose a compact
Reconnect control for a cancelled/disconnected session. The control calls a
dedicated reconnect callback, not the auto-connect effect.
...
An upstream terminal cancellation pauses the per-tab message queue. The queue
remains visible and editable, but it is not drained merely because a reconnect
reaches `connected`.

Reconnect alone keeps the queue paused. A direct new send is an explicit new
turn and bypasses paused historical queue items. Resuming queued items is a
separate explicit command that restores the existing FIFO flush behavior. This
prevents cancellation from silently becoming a retry while retaining the
user's unsent drafts.
...
| User sends a new prompt | `in_progress` | prompting | unchanged historical result | new turn only |
...
If global delivery was unavailable while the terminal event occurred,
...
1. the durable row transitions from `in_progress` to `cancelled`;
...
1. an active persisted `cancelled` tab does not call `connect` on mount,
   remount, focus, or transport reconciliation;
2. an explicit Reconnect command calls `connect` once with the existing
   session identity without changing the persisted status;
3. a connected ACP projection does not auto-flush a queue while the root is
   cancelled;
4. a direct user send after reconnect is allowed and transitions through the
   existing in-progress path; and
5. explicitly resuming the queue restores FIFO flush behavior.
...
Existing non-terminal sessions connect after the status source resolves, which
is a short, safe delay.
